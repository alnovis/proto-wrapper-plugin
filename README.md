...
- Type conflict handling between versions (int→Long, incompatible types)
...
| `name` | Version name (defaults to uppercase of `protoDir`, e.g., `v1` → `V1`) |
...
| Situation | Solution |
|-----------|----------|
| `int` in v1 → `long` in v2 | Uses `long`, v1 applies cast `(long)` |
| `int` in v1 → `enum` in v2 | Uses `int`, enum converts via `getNumber()` |
...
┌─────────────────────────────────────────────────────────────┐
│                    ProtoAnalyzer                            │
│          Parses descriptors into VersionSchema              │
│          Filters by sourcePrefix (directories)              │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│                    VersionMerger                            │
│         Merges schemas into unified MergedSchema            │
│         Detects equivalent enums                            │
