 ...
 * @see io.alnovis.protowrapper.model.ConflictEnumInfo
 */
public final class IntEnumHandler extends AbstractConflictHandler implements ConflictHandler {
    ...
    public void addConcreteBuilderMethods(TypeSpec.Builder builder, MergedField field,
        ...
        java.util.List<String> sortedVersions = enumVersions.stream()
                .sorted()
                .toList();
        ...
        if (!sortedVersions.isEmpty()) {
            String versionCondition = sortedVersions.stream()
                    .map(v -> "getVersionId().equals(\"" + v + "\")")
                    .reduce((a, b) -> a + " || " + b)
                    .orElse("false");
