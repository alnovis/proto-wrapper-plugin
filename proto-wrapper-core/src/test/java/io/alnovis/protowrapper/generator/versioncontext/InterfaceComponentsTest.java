...
class InterfaceComponentsTest {
    ...
    class StaticMethodsComponentTest {
        ...
        @Test
        @DisplayName("does not add deprecated forVersion method")
        void doesNotAddDeprecatedForVersionMethod() {
            new StaticMethodsComponent(config, schema).addTo(builder);

            String code = builder.build().toString();
            // Deprecated forVersion(int) should NOT be generated
            assertThat(code).doesNotContain("forVersion(int version)");
            assertThat(code).doesNotContain("switch (version)");
        }
    }
    ...
    class InstanceMethodsComponentTest {
        ...
        @Test
        @DisplayName("does not add deprecated getVersion method")
        void doesNotAddDeprecatedGetVersionMethod() {
            new InstanceMethodsComponent(config, schema).addTo(builder);
    ...
    class WrapMethodsComponentTest {
        ...
        void generatesDefaultMethodForVersionSpecificMessages() {
            ...
            FieldDescriptorProto fieldProto = FieldDescriptorProto.newBuilder()
                    ...
                    .setLabel(Label.LABEL_OPTIONAL)
                    .build();
            v2Only.addField(MergedField.builder().addVersionField("v2", new FieldInfo(fieldProto)).build());
